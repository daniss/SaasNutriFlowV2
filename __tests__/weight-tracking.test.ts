...
const mockSupabase = {
  from: (table: string) => ({
    ...
    update: (data: any) => ({
      eq: (field: string, value: string) => ({ data: { ...data }, error: null })
    }),
    delete: () => ({
...
describe('Weight Tracking System', () => {
  ...
  it('should update current_weight to previous measurement when deleting', async () => {
    ...
    const expectedCurrentWeight = remainingEntries[remainingEntries.length - 1].weight
  ...
  it('should handle empty weight history when deleting last measurement', async () => {
    ...
    const expectedCurrentWeight = remainingEntries.length > 0 
